...
namespace wke
{
	...
	public:
		//add by rrrfff
		//  [1/15/2015 ASUS]
		wkeEx::ON_CONSOLE_OUTPUT onConsoleWrite;
		wkeEx::ON_JS_ALERT onJsAlert;
		wkeEx::ON_JS_CONFIRM onJsConfirm;
		wkeEx::ON_JS_PROMPT onJsPrompt;
    ...
    public:
        ...
        virtual void addMessageToConsole(WebCore::MessageSource src, WebCore::MessageType mt, WebCore::MessageLevel ml, const WTF::String& message, unsigned int lineNumber, const WTF::String& url) override
        {           
			if (this->onConsoleWrite != nullptr){
				this->onConsoleWrite(static_cast<wkeEx::MessageSource>(src),
									  static_cast<wkeEx::MessageType>(mt),
									  static_cast<wkeEx::MessageLevel>(ml),
									  CSTR(message), lineNumber, CSTR(url));
			}
			else{
				dbgMsg(L"console message %s %d %s\n", CSTR(message), lineNumber, CSTR(url));
			}
        }
        ...
        {
			if (this->onJsAlert != nullptr){
				this->onJsAlert(CSTR(msg));
			}
			else{
				dbgMsg(L"JavaScript Alert %s\n", CSTR(msg));
			}
        }

        virtual bool runJavaScriptConfirm(WebCore::Frame*, const WTF::String& msg) override
        {
			if (this->onJsConfirm != nullptr){
				return this->onJsConfirm(CSTR(msg));
			}
			else{
				dbgMsg(L"JavaScript Confirm %s\n", CSTR(msg));
				return true;
			}           
        }
        ...
        {
			if (this->onJsPrompt != nullptr){
				wchar_t prompt_result[256] = {0};
				bool _result = this->onJsPrompt(CSTR(msg), CSTR(defaultValue), prompt_result);
				result = prompt_result;
				return _result;
			}
			else{
				dbgMsg(L"JavaScript Prompt %s %s\n", CSTR(msg), CSTR(defaultValue));
				return true;
			}         
        }
        ...
        virtual void runOpenPanel(WebCore::Frame*, PassRefPtr<WebCore::FileChooser> chooser) override
        {
			...
			ofn.lpstrFile = files;
			ofn.nMaxFile = sizeof(files) / sizeof(TCHAR) - 1;
			ofn.Flags = OFN_EXPLORER | OFN_FILEMUSTEXIST;
			//currently, we don't support OFN_ALLOWMULTISELECT
			//by rrrfff
			//  [1/17/2015 ASUS]

			if (GetOpenFileName(&ofn)){
				chooser->chooseFile(files);
				//chooser->settings().selectedFiles.append(files);
			}			
        }
