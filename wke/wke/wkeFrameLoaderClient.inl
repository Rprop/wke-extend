...
namespace wke
{
    ...
    public:
        ...
        virtual void dispatchDidCommitLoad() override
        {
			...
			//changed by rrrfff
			//  [1/15/2015 ASUS]
			if (!handler->onURLChanged(handler, (const wkeString)&url.string())){
				loader->stopLoading();
			}
        }
        ...
        virtual void dispatchDidFinishDocumentLoad() override
        {
			...
			const wkeClientHandler* handler = webView_->getClientHandler();
			if (handler == NULL || handler->onDocumentReady == NULL)
				return;
			...
			handler->onDocumentReady(handler, (const wkeString)&url.string());
        }
