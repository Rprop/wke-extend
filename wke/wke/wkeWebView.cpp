...
//add by rrrfff
//  [1/15/2015 ASUS]
#define IMPLEMENT_HANDLER(c,t,a,s,g) void c::set##a(const t handler){s;}; \
	const t c::get##a() const{g;}
...
namespace wke
{
		...
		,bufHandler_(NULL)
    {
		//add by rrrfff
		//  [1/15/2015 ASUS]
		client_ = new ChromeClient(this);

        WebCore::Page::PageClients pageClients;
        pageClients.chromeClient = client_;
		...
		//changed by rrrfff
		//  [1/15/2015 ASUS]
		if (wkeEx::LocalStorageDatabasePath[0] == 0)
		{
			GetCurrentDirectory(MAX_PATH - sizeof("\\localStorage"), wkeEx::LocalStorageDatabasePath);
			wcscat(wkeEx::LocalStorageDatabasePath, L"\\localStorage");
		} //if
        
		settings->setLocalStorageDatabasePath(wkeEx::LocalStorageDatabasePath);
		WebCore::DatabaseTracker::tracker().setDatabaseDirectoryPath(wkeEx::LocalStorageDatabasePath);
	...
	IMPLEMENT_HANDLER(CWebView, wkeClientHandler *, ClientHandler, clientHandler_ = handler, return clientHandler_);
	IMPLEMENT_HANDLER(CWebView, wkeBufHandler *, BufHandler, bufHandler_ = const_cast<wkeBufHandler *>(handler), return bufHandler_);

	//add by rrrfff
	//  [1/15/2015 ASUS]
	IMPLEMENT_HANDLER(CWebView, wkeEx::ON_CONSOLE_OUTPUT, ConsoleOutputHandler, client_->onConsoleWrite = handler, return client_->onConsoleWrite);
	IMPLEMENT_HANDLER(CWebView, wkeEx::ON_JS_ALERT, JsAlertHandler, client_->onJsAlert = handler, return client_->onJsAlert);
	IMPLEMENT_HANDLER(CWebView, wkeEx::ON_JS_CONFIRM, JsConfirmHandler, client_->onJsConfirm = handler, return client_->onJsConfirm);
	IMPLEMENT_HANDLER(CWebView, wkeEx::ON_JS_PROMPT, JsPromptHandler, client_->onJsPrompt = handler, return client_->onJsPrompt);
}
