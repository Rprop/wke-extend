...
#   define WKE_SHARED_DATA __declspec(dllexport)
#else
#   define WKE_API         __declspec(dllimport)
#   define WKE_SHARED_DATA __declspec(dllimport)
#endif
...
namespace wkeEx
{
	...
	typedef void(*ON_CONSOLE_OUTPUT)(MessageSource, MessageType, MessageLevel,
									 const wchar_t *message, unsigned int lineNumber,
									 const wchar_t *url);
	typedef void(*ON_JS_ALERT)(const wchar_t *msg);
	typedef bool(*ON_JS_CONFIRM)(const wchar_t *msg);
	/*
	 *	@warning the max length of result is 255 
	 */
	typedef bool(*ON_JS_PROMPT)(const wchar_t *msg, const wchar_t *defaultValue,/* OUT */wchar_t *result);
	
	/*
	 *	get or set the LocalStorageDatabasePath
	 *	@warning thread unsafe
	 */
	WKE_SHARED_DATA extern wchar_t LocalStorageDatabasePath[260];
	/*
	*	get or set the User-Agent
	*	@warning thread unsafe
	*/
	WKE_SHARED_DATA extern wchar_t UserAgent[256];
};
...
//changed by rrrfff
//  [1/15/2015 ASUS]
//return false to stop current loading
typedef bool (*ON_URL_CHANGED) (const struct _wkeClientHandler* clientHandler, const wkeString url);
//add by rrrfff
//  [1/15/2015 ASUS]
typedef void(*ON_DOCUMENT_READY) (const struct _wkeClientHandler* clientHandler, const wkeString url);
...
{
	virtual void onBufUpdated(const struct HDC__* hdc, int x, int y, int cx, int cy) = 0;
}wkeBufHandler;
...
//add by rrrfff
//  [1/15/2015 ASUS]
#define DECLARE_HANDLER(t,a) virtual void set##a(const t handler); virtual const t get##a() const;
#define DECLARE_HANDLER_INTERFACE(t,a) virtual void set##a(const t handler) = 0; virtual const t get##a() const = 0;
...
namespace wke
{
    ...
    public:
		...
		virtual struct HDC__* getViewDC() = 0;
		...
		DECLARE_HANDLER_INTERFACE(wkeClientHandler *, ClientHandler);
		DECLARE_HANDLER_INTERFACE(wkeBufHandler *, BufHandler);

		//add by rrrfff
		//  [1/15/2015 ASUS]
		DECLARE_HANDLER_INTERFACE(wkeEx::ON_CONSOLE_OUTPUT, ConsoleOutputHandler);
		DECLARE_HANDLER_INTERFACE(wkeEx::ON_JS_ALERT, JsAlertHandler);
		DECLARE_HANDLER_INTERFACE(wkeEx::ON_JS_CONFIRM, JsConfirmHandler);
		DECLARE_HANDLER_INTERFACE(wkeEx::ON_JS_PROMPT, JsPromptHandler);
    };
}
...
//added by rrrfff
//  [1/17/2015 ASUS]
WKE_API void wkeInitMainThread();
//added by rrrfff
//  [1/25/2015 ASUS]
WKE_API void wkeSetCookieJarFileName(const char *cookieJarFileName);
WKE_API void wkeSetProxyInfo(const char *host, unsigned long port,
...
WKE_API void wkeSetFileSystem(WKE_FILE_OPEN pfn_open, WKE_FILE_CLOSE pfn_close, WKE_FILE_SIZE pfn_size, WKE_FILE_READ pfn_read, WKE_FILE_SEEK pfn_seek);
